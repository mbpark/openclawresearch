...
# CheckpointManager must be defined before SecureCheckpointManager
...
class ResilientService:
    ...
    def __init__(self, name: str, checkpoint_manager: Optional[CheckpointManager] = None):
        self.name = name
        ...
        self.checkpointing = Checkpointing(checkpoint_manager or CheckpointManager(), "service_state")
        self.service_state = {
            "request_count": 0,
            "error_count": 0,
    ...
    async def process_request(self, request_id: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async def operation():
            ...
            self.service_state["last_request_id"] = request_id
            ...
            if random.random() < 0.2:
                self.service_state["error_count"] += 1
            ...
            return {
                ...
                "payload": payload,
                "timestamp": time.time(),
                "service_state": self.service_state.copy()
            }

        # Apply resilience patterns with checkpointing
        return await self.checkpointing.execute(
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "checkpoint_count": len(self.checkpointing.checkpoint_manager.checkpoints)
        }
...
async def main():
    ...
    print("=" * 60)
    print("Resilience Patterns Testing")
    print("=" * 60)
    ...
    try:
        result = await service.process_request("req_1", {"amount": 100})
        print(f"✅ Success: {result}")
    except Exception as e:
        print(f"❌ Failed: {e}")

    # Test with multiple failures
    print("\n2. Testing with multiple failures:")
    for i in range(5):
        try:
            result = await service.process_request(f"req_{i+2}", {"amount": i * 10})
            print(f"✅ Request {i+2} success")
        except Exception as e:
            print(f"❌ Request {i+2} failed: {e}")
    ...
    print("\n" + "=" * 60)
    print("All resilience patterns tested successfully!")
    print("=" * 60)
